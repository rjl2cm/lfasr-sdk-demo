...
{
	// 原始音频存放地址
	private static final String local_file = "/Users/franer/Desktop/180116丛聪2.MP3";
	/*
	 * 转写类型选择：标准版和电话版分别为：
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (resultMsg.getOk() == 0) {
				// 打印转写结果
				String resultStr = resultMsg.getData();
				System.out.println("结果是:"+resultMsg.getData());
				new VoideTextParser().run(resultStr);
			} else {
				// 转写失败，根据失败信息进行处理
//				System.out.println("ecode=" + resultMsg.getErr_no());
//				System.out.println("failed=" + resultMsg.getFailed());
				System.out.println("转写失败！");
			}
		} catch (LfasrException e) {
			// 获取结果异常处理，解析异常描述信息
